...
## 2026-09-24 — 分享資料夾一鍵下載

- 分享頁新增「下載整個分享資料夾」，直接以原生表單下載 ZIP，無須逐項勾選。
- 新增 `POST /api/files/shared/archive-all`，驗證分享碼及資料夾類型後，以既有串流 ZIP 打包分享根目錄及子目錄；維持 2000 項目與 4 GiB 上限。
- 新增前後端測試，涵蓋一鍵送出、單檔分享拒絕、無效與過期分享碼；`npm run check` 通過（292 項測試）。
- 更新 README 與檔案功能說明，重建已納入版本控制的 `website-part/public`。

### 修正下載按鈕沒有反應

- 分享頁原本把下載表單送進隱藏 iframe；伺服器若回傳錯誤，頁面無法顯示。改為由目前分頁接收下載，成功時瀏覽器照常儲存附件，失敗時可看到伺服器回應。
- 在 Router 上以臨時分享碼對有效分享相同路徑實測，`archive-all` 回應 `200 application/zip`，ZIP 的檔頭與結尾記錄正確；臨時分享已撤銷。
- 此次修正涵蓋整個資料夾、勾選打包與單檔下載的表單行為。
